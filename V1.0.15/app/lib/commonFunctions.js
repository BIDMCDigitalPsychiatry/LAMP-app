...
exports.handleException = function(view, functionName, exception, customMsg) {
	try {
		...
		//var ErrorMessage = "An unexpected error has occurred. Please retry later.";
		...
		// serviceManager.submitError(message);
	} catch(ex) {
...
// var text = message;
// Ti.API.info('initActivityIndicator');
// // 'Loading ...';
// var windowTop = 0;
...
exports.formatTimeToDate = function(time, date, second) {
	...
	// month = "0" + month;
	//
	// var setTime =myDate.getFullYear()+"-" + month + "-" + day + "T" + time ;
	// Ti.API.info('SET TIME :: ', setTime);
	//
	//
	//
	// //var temDate = setTime + "T" + time;
	//
	// //Ti.API.info('temDate : ', temDate);
	// var actualDate = new Date(setTime);
	// Ti.API.info('actualDate is', actualDate);
	// //var actualTime = time.split(':');
	// //Ti.API.info('actualTime[0] : ', actualTime[0]);
	//
...
exports.getMonthDays = function(pause, resume, setTime) {
	...
	while (d <= to) {
		...
		// var resumeTime = to;
		// var dateSet = new Date(resumeTime);
		// dateSet.setHours(notTime.getHours(), notTime.getMinutes(), notTime.getSeconds());
		// var d1 = d;
		// var t1 = new Date(to);
		// t1.setHours(d.getHours(), d.getMinutes(), d.getSeconds(), d.getMilliseconds());
		// Ti.API.info('D1:: ', d1.getTime());
		// Ti.API.info('T1:: ', t1.getTime());
		// Ti.API.info('resumeTimeT1: ', resumeTime.getTime());
		// Ti.API.info('dataset: ', dateSet.getTime());
		// //d1.getTime() == t1.getTime() &&
		// if ((resumeTime.getTime() < dateSet.getTime() && d1.getDate() == t1.getDate() && d1.getMonth() == t1.getMonth()) || d1.getTime() > dateSet.getTime()) {
		// Ti.API.info('1111');
		// dayCount = dayCount - 1;
		// }
		// d = new Date(d.getTime() + (24 * 60 * 60 * 1000));
...
exports.getMonthDaysNew = function(pause, resume, setTime) {
	...
	var to = new Date(resume);
	var notTime = new Date(setTime);
	//alert("notTime.getDate() : " + notTime.getDate());
	var dayCount = 0;
	var d = from;
	if (from.getTime() < notTime.getTime())
		d = notTime;
...
exports.getDailyDays = function(pause, resume, dayName, setTime) {
	...
	// var dateSet = new Date(resumeTime);
	// dateSet.setHours(notTime.getHours(), notTime.getMinutes(), notTime.getSeconds());
	// Ti.API.info('***DATESET*** ', dateSet);
	// Ti.API.info('paused time****** ', d);
	// var d1 = d;
	// var t1 = new Date(to);
	// t1.setHours(d.getHours(), d.getMinutes(), d.getSeconds(), d.getMilliseconds());
	// Ti.API.info('D1:: ', d1.getTime());
...
exports.getHoursNumber = function(pause, resume, setTime, index, type, isAdmin) {
	...
	if (index == 0) {
		...
		//var no0fHours = Math.trunc(diff / 3600);
		...
		var no0fHours = parseInt(arrTemp[0]);
		//var no0fHours = Math.trunc(diff / 21600);
	} else if (index == 3) {
		var tempTest = diff / 43200;
		var arrTemp = tempTest.toString().split(".");

		var no0fHours = parseInt(arrTemp[0]);
		//var no0fHours = Math.trunc(diff / 43200);
	}
	...
	if (lastDate != "") {
		...
		// if (type == "survey") {
		//
		// //var lastReminderTime = Ti.App.Properties.getString('lastReminderTimeSurvey', "");
		// Ti.App.Properties.setString('lastReminderTimeSurvey', lastDate.toUTCString());
		//
		// } else {
		// Ti.App.Properties.setString('lastReminderTimeCog', lastDate.toUTCString());
		// //var lastReminderTime = Ti.App.Properties.getString('lastReminderTimeCog', "");
		// }

		//Ti.App.Properties.setString('lastReminderTime', lastDate.toUTCString());
	}
...
exports.getScoreViewJewelTotal = function(gameScore, points, timeTaken) {
	...
	if (Ti.Platform.osname == "ipad") {
		...
		if (OS_ANDROID) {
			Ti.API.info('enter total score android');
			var screenWidthInInches = Titanium.Platform.displayCaps.platformWidth / Titanium.Platform.displayCaps.dpi;
			var screenHeightInInches = Titanium.Platform.displayCaps.platformHeight / Titanium.Platform.displayCaps.dpi;
			var maxInches = (screenWidthInInches >= screenHeightInInches) ? screenWidthInInches : screenHeightInInches;
			Ti.API.info('maxInches : ', maxInches);
	...
	button1.addEventListener('click', function() {
		Ti.API.info('button1 click: ');

		// win.close(innerView);
		//win.close(outerView);
		win.remove(innerView);
		win.remove(outerView);
		win.close();
		win = null;
...
exports.protoTypeReminder = function() {
	...
	if (memoryTime != "") {
		...
		// if (diffDays >= 6) {
		// remaining = diffDays % 6;
		// }

		//Ti.API.info('remaining is', remaining);
		...
		if (diffDays == 6) {
			...
			// var reduceDate = diffDays - 8;
			// if (reduceDate < 7) {
			// if (reduceDate > 0) {
			// var d = new Date();
			// d.setDate(d.getDate() - reduceDate);
			// Ti.App.Properties.setString('memoryTime', d.toUTCString());
			// } else {
			// Ti.App.Properties.setString('memoryTime', new Date().toUTCString());
			// }
			//
			// } else {
			// Ti.App.Properties.setString('initialTime', new Date().toUTCString());
			// Ti.App.Properties.setString('memoryTime', new Date().toUTCString());
			//
			// }
	...
	if (initialTime != "") {
		...
		if (diffDays == 11) {
			...
			// var reduceDate = diffDays - 40;
			// if (reduceDate < 39) {
			// if (reduceDate > 0) {
			// var d = new Date();
			// d.setDate(d.getDate() - reduceDate);
			// Ti.App.Properties.setString('initialTime', d.toUTCString());
			// } else {
			// Ti.App.Properties.setString('initialTime', new Date().toUTCString());
			// }
			//
	...
	try {
		...
		if (earnedBonus > jewelTrailsBSettingsJSON["NoOfBonusPoints"]) {
			...
			if (currentLevel % jewelTrailsBSettingsJSON["Y_NoOfChangesInLevel"] == 0) {
				...
				//diamondShapeToDisplay = jewelTrailsBSettingsJSON["NoOfShapes"] + ((Math.floor(currentLevel / jewelTrailsBSettingsJSON["Y_NoOfChangesInLevel"])) * jewelTrailsBSettingsJSON["Y_NoOfShapes"]);
...
exports.L = function(str, value) {
	try {
		if (OS_IOS) {
			var file = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory, '/language/' + value + '.xml');
			//var file = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory, 'i18n/en.xml');
			//var file = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory, '/i18n/en.xml');
			if (file.exists()) {
				var xmltext = file.read().text;
...
exports.getGameID = function(Game) {
	var gameID;
	Ti.API.info('Game', Game);
	var commonFunctions = require('commonFunctions');
	var LangCode = Ti.App.Properties.getString('languageCode');
	if (Game == commonFunctions.L('nbackTest', LangCode)) {
		gameID = 1;
	} else if (Game == commonFunctions.L('trailsBTest', LangCode)) {
		gameID = 2;
	} else if (Game == commonFunctions.L('spatialFrwd', LangCode)) {
		gameID = 3;
	} else if (Game == commonFunctions.L('spatialBckwrd', LangCode)) {
		gameID = 4;
	} else if (Game == commonFunctions.L('memoryTest', LangCode)) {
		gameID = 5;
	} else if (Game == commonFunctions.L('serial7', LangCode)) {
		gameID = 6;
	} else if (Game == commonFunctions.L('figCopy', LangCode)) {
		gameID = 8;
	} else if (Game == commonFunctions.L('visualGame', LangCode)) {
		gameID = 9;
	} else if (Game == commonFunctions.L('digitFWrd', LangCode)) {
		gameID = 10;
	} else if (Game == commonFunctions.L('digitBckWrd', LangCode)) {
		gameID = 13;
	} else if (Game == commonFunctions.L('catdog', LangCode)) {
		gameID = 11;
	} else if (Game == commonFunctions.L('temporalOrder', LangCode)) {
		gameID = 12;
	} else if (Game == commonFunctions.L('trailsBTestNew', LangCode)) {
		gameID = 15;
	} else if (Game == commonFunctions.L('trailsBTouch', LangCode)) {
		gameID = 16;
	} else if (Game == commonFunctions.L('jwelA', LangCode)) {
		gameID = 17;
	} else if (Game == commonFunctions.L('jwelB', LangCode)) {
		gameID = 18;
	} else if (Game == commonFunctions.L('nbackTestNew', LangCode)) {
		gameID = 14;
	}
 ...
 * Get game name
 */
exports.getGameNameFromId = function(GameID) {
	var gameName;
	var commonFunctions = require('commonFunctions');
	Ti.API.info('GameID in common fucntion', GameID);
	var LangCode = Ti.App.Properties.getString('languageCode');
	if (GameID == 1) {
		gameName = commonFunctions.L('nbackTest', LangCode);
	} else if (GameID == 2) {
