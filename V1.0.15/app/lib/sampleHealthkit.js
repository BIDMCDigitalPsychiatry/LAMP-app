...
function getSleepAnalysis(successCallBack) {
	try {
		...
		TiHealthkit.getCategoryQuantityResult(query, function(data) {
			if (data.success == 1) {
				Ti.API.info('SLEEP count = '+ data);
				if (data.quantities != null) {
					var totalSleepTime = 0;
					Ti.API.info('getSleepAnalysis forloop');
					for (var i = 0; i < data.quantities.length; i++) {
						var strtDate = new Date(data.quantities[i].startDate).getTime();
						var endDate = new Date(data.quantities[i].endDate).getTime();
						Ti.API.info('strtDate', new Date(data.quantities[i].startDate).getDate());
						Ti.API.info('endDate', endDate);
						var sleepTime = endDate - strtDate;
						if (new Date(data.quantities[i].startDate).getDate() == new Date().getDate()) {
							totalSleepTime = totalSleepTime + sleepTime;
							Ti.API.info('totalSleepTime', totalSleepTime);
						}
		...
		TiHealthkit.getQuantityResult(query, function(data) {
			...
			if (data.success == 1) {
				if (data.quantities != null) {
					...
					Ti.API.info('StepValue in step', stepCount);
