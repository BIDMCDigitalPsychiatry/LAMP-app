...
function init() {
	if (OS_IOS) {
		if (Alloy.Globals.DEVICE_HEIGHT >= 812) {
			...
			$.buttonContainerView.bottom = "28dp";
...
function setLabel() {
	...
	$.signInStudent.text = commonFunctions.L('signInSmall', LangCode);
...
$.signinStudy.addEventListener("open", function(e) {
	Ti.API.info('isFirstTimeLogin in open', args.isFrom);
	if (args.isFrom == "passwordScreen") {
		$.signInStudyButton.visible = true;
		$.signInStudent.visible = true;
		$.signInUserSignUp.visible = true;
		$.headerView.setLeftViewVisibility(false);
	} else if (args.isFrom == "signin") {
		$.signInStudyButton.visible = true;
		$.signInStudent.visible = false;
		$.signInUserSignUp.visible = false;
		$.headerView.setLeftViewVisibility(true);
	}
	setLabel();
	setTimeout(function() {
		enableAllTExtBoxes();
