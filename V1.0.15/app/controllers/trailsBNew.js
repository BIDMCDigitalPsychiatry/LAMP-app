...
Ti.API.info('pWidth out *****', pWidth,Ti.Platform.displayCaps.platformWidth,Ti.Platform.displayCaps.dpi);
Ti.API.info('pHeight out *****', pHeight);
		
		Ti.API.info('widthValue out *****', widthValue);
		var fullArrayPositions = generatePositionsArray(pWidth, pHeight, 80, 10);
		Ti.API.info('fullArrayPositions out *****', fullArrayPositions);
		if (levelNumb == 1) {
			Ti.API.info('fullArrayPositions out 1');
			var selectedAlphabetsArray = ['A', 'B', 'C', 'D'];
			var selectedNumbers = ['1', '2', '3', '4'];
			arrayPositions = getRandomPosition(fullArrayPositions, 8);
			Ti.API.info('arrayPositions out *****', arrayPositions);
		} else if (levelNumb == 2) {
			Ti.API.info('fullArrayPositions out 2');
			var selectedAlphabetsArray = ['A', 'B', 'C', 'D', 'E'];
			var selectedNumbers = ['1', '2', '3', '4', '5'];
			arrayPositions = getRandomPosition(fullArrayPositions, 10);
		} else if (levelNumb == 3) {
			Ti.API.info('fullArrayPositions out 3');
		...
		Ti.API.info('arrayPositions out');
		tempArrPositions = arrayPositions.slice();
		Ti.API.info('tempArrPositions out *****', tempArrPositions);
		var oddIndex = 0;
		var evenIndex = 0;
					...
					if (gameStarted == 1) {
						...
						if (targetIndex == tempIndex) {
							...
							if (levelNumb == 3) {
								...
								Ti.API.info('totalgameAttempt : ', totalgameAttempt, correctLines);
								//var attempt = totalgameAttempt - 1;
								if (totalgameAttempt < 30) {
									var gameScore = Math.round((correctLines / 30) * 100);
								} else {
									var gameScore = Math.round((correctLines / totalgameAttempt) * 100);
...
function getRandomPosition(arr, n) {
	try {
		Ti.API.info('getRandomPosition*****', JSON.stringify(arr));
		var tempArray = [];
		var result = [];
		var len = arr.length;
		var indexValue = 0;
		while (tempArray.length < n) {
			Ti.API.info('len n *****', tempArray.length, len);	
			var randomnumber = Math.floor(Math.random() * len);
			Ti.API.info('randomnumber n *****', randomnumber);
...
function getValues() {
	...
	if (Ti.Network.online) {
		...
		// var gameScore = Math.round((correctLines / 30) * 100);
		// } else {
		// var gameScore = Math.round((correctLines / attempt) * 100);
		// }
