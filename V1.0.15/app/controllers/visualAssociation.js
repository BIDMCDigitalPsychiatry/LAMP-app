	...
	try {
		...
		if (args.reminderVersion != null && args.reminderVersion != 0) {
			...
			if (vNumber == "") {
				...
				//Ti.App.Properties.setString('GameVersionNumber', 2);
				vNumber = 1;
				...
				//Ti.App.Properties.setString('GameVersionNumber', 3);
			} else if (vNumber == 3 || vNumber == "3") {
				versionsInfo.VisualAssociation = 4;
				//Ti.App.Properties.setString('GameVersionNumber', 4);
		...
		//$.levelLabel.text = "Version " + vNumber;
...
function gameLogic() {
	...
	memoryTimer = setInterval(function() {
		...
		if (currentImageIndex == 5) {
			...
			//var randomInd = getRandomInt(1, 2);
...
function resultOnclick(e) {
	try {
		...
		if (e.source.index != null) {
			...
			//Ti.API.info('thumbImageArrayOrg[indexValue - 1] : ', thumbImageArrayOrg[indexValue - 1].index, thumbImageArrayOrg[indexValue - 1].image);
...
function nextLevel() {
	...
	if (selectedImageArray.length == currentImageIndex) {
		...
		//	var wrongAnswers = 5 - CorrectAnswers;
