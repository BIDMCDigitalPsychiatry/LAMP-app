...
$.scoreTable.addEventListener("open", function(e) {
	try {
		...
		Ti.API.info('gameResults is' + JSON.stringify(gameResults));
		var gameList = [{
		...
		for (var i = 0; i < gameResults.length; i++) {
			Ti.API.info('gameList.length : ', gameList.length, gameResults[i].gameID);
		...
		Ti.API.info('game info from DB is' + JSON.stringify(gameList));
		loadData(gameList);
	} catch(ex) {
...
function loadData(gameInfo) {
	for (var i = 0; i < gameInfo.length; i++) {
		...
		if (gameInfo[i].gameID == 1) {
			...
			//$.catDogLbl.value = gameInfo[i].score;
