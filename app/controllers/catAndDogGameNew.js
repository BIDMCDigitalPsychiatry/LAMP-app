...
function boxSelectionClick(e) {
	...
	if (e.source.index != null) {
		...
		Ti.API.info('indexValue : ', indexValue, isCorrect);
		if (isCorrect == true) {
		...
		tapIndex += 1;
		Ti.API.info('tapIndex : ', tapIndex, " numberOfCorrectImage : ", numberOfCorrectImage);
		if (numberOfCorrectImage == tapIndex) {
			Ti.API.info('nextButton Visible');
			userActionEnable = false;
...
function onNextClick(isAutomatic) {
	...
	if (stage != compareStage) {
		...
		Ti.API.info('gameResultArray', JSON.stringify(gameResultArray));
		if (level < 3) {
			correctCount = 0;
			wrongCount = 0;
			endTimeDiffrnce = "";
...
function getValues() {
	...
	if (Ti.Network.online) {
		...
		if (args.fromNotification === true && args.isLocal != 1) {
			...
			spinWheelScore = 5;
			Ti.API.info('****SpinWheelScore ****' + spinWheelScore);
		}
