...
$.refreshPreventScreen = function(e) {
	try {
		Ti.API.info(' ****** refreshPreventScreen *******');
...
function getGameScoreSuccess(e) {
	try {
		...
		Ti.API.info('*** SPIN GAME TOTAL SUCCESS ****  ', JSON.stringify(response));
		if (response.ErrorCode == 0) {
			...
			if (response.DayStreak != null && response.DayStreak != 0) {
				if (response.GameDate == "" || response.GameDate == null) {
					...
					var dateDifference = parseInt(new Date().getDate()) - parseInt(gameDate.getDate());
					if (dateDifference > 1) {
						dayStreak = 0;
					}
				}
			...
			if (noOFRecords == 0) {
				...
				Ti.API.info('*********** spinScoreTime ********* ' + spinScoreTime);
				if (spinScoreTime != "") {
					var spinScoreDate = spinScoreTime;
					Ti.API.info('*********** spinScoreDate ********** ' + spinScoreDate);
			...
			getGoldStars();
			Ti.API.info('spinWheelScreen goldenStarsCollected = ' + goldenStarsCollected);
			Ti.API.info('spinWheelScreen blueStarCollected = ' + blueStarsCollected);
			Ti.API.info('userTotalScore = ' + userTotalScore);
			if (userTotalScore >= 75) {
				var spinInfo = Ti.App.Properties.getObject('spinnerInfo');
				var completedSpin = streakSpin;
				Ti.API.info(' ******* completedSpin *******' + completedSpin);
				...
				Ti.API.info(' ******* spinForToday *******' + spinForToday);
				if (!OS_IOS) {
