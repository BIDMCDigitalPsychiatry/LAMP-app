	...
	try {
		...
		if (args.isForward === true) {
			...
			if (Ti.Platform.osname == "ipad") {
				...
				$.headerView.setTitle(commonFunctions.trimText(titleText, 16));
			}
		} else {
			...
			if (Ti.Platform.osname == "ipad") {
				...
				$.headerView.setTitle(commonFunctions.trimText(titleText, 17));
			}
		}
...
function gameLogic() {
	...
	var i = 0;
	nextLevelEnable = false;
	Ti.API.info('arrayBoxes length : ', arrayBoxes.length);
...
/**
 * boxSelectionClick event handler
 */
function boxSelectionClick(e) {
	...
	Ti.API.info('Start Tapping');
	Ti.API.info('Start time&&&&&&&&&&&&&&&&&&&&&&&&', startTimeDiffrnce);
	...
	if (e.source.index != null) {
		...
		Ti.API.info('TimeTaken *******************************', TimeTaken);
		boxesArray.push({
...
function onNextClick(e) {
	...
	Ti.API.info('isAnyWrong : ', isAnyWrong, numberOfwrongStanges);
...
function getValues() {
	...
	if (Ti.Network.online) {
		...
		Ti.API.info('param of spatial is', JSON.stringify(resultParameter));
...
function onSaveSpatialSpanGameSuccess(e) {
	try {
		...
		Ti.API.info('***SAVE SPATIAL SPAN GAME SUCCESS  RESPONSE****  ', JSON.stringify(response));
