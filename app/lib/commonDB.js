		...
		for (var i = 0; i < surveyDatas.length; i++) {
			...
			if (!sqlResultSet.isValidRow()) {
				...
				if (surveyDatas[i].IsDeleted == false) {
					db.execute('INSERT INTO Survey(userId,answerType,surveyId,questionId,questionText,language)VALUES (?,?,?,?,?,?)', userId, ansType, surveyID, surveyDatas[i].QuestionId, surveyDatas[i].QuestionText, languageCode);
					if (ansType == 2) {
				...
				if (surveyDatas[i].IsDeleted == false) {
					db.execute('UPDATE Survey SET answerType =?,questionText=?,language=? where questionId = ? AND userId=?', ansType, surveyDatas[i].QuestionText, languageCode, surveyDatas[i].QuestionId, userId);
					if (ansType == 2) {
 ...
 */
exports.insertSurveyTypes = function(surveyID, surveyName, userId, IsDeleted) {
	try {
		var db = Ti.Database.open(Alloy.Globals.DATABASE);
		db.execute('BEGIN;');
		...
		if (!sqlResultSet.isValidRow()) {
			...
			if (IsDeleted == false) {
				...
				db.execute('INSERT INTO SurveyListings(userId,surveyId,surveyName)VALUES (?,?,?)', userId, surveyID, surveyName);
			}

		} else {
			Ti.API.info('update survey', surveyName, IsDeleted);
			if (IsDeleted == false) {
				db.execute('UPDATE SurveyListings SET surveyName =? where surveyId = ? AND userId=?', surveyName, surveyID, userId);
			} else {
...
exports.insertAlerts = function(userId, alertbody, type, version, pTestId, pTestName, pIsAdmin, batchID) {
	try {
		...
		if (pIsAdmin == 1) {
			...
			var alertRows = db.execute('SELECT * FROM Alerts where testID=' + pTestId + ' AND isLocal=' + 0);
