...
/**
 * onDistractionSurveySuccess event handler
 */
...
/**
 * getSurveyListSuccess event handler
 */
...
function gameLogic() {
	if (count == 1) {
		Ti.API.info('enter count');
		currentImageIndex = 0;
		viewImages();
	} else {
		Ti.API.info('enter else of  survey screen ', srartIndex);
		var imgPathSample = "/images/simpleMemory/" + vNumber + "/" + srartIndex + ".png";
		Ti.API.info('Image path : ', imgPathSample);
...
function viewImages() {
	...
	Ti.API.info('@@@@length currentIndex_view images@@@@' + currentImageIndex);
...
function saveUserSurveySuccess() {
	try {
		commonFunctions.closeActivityIndicator();
		Ti.API.info('*** SimpleMemory saveUserSurveySuccess');
	} catch(ex) {
