...
$.signin.addEventListener("open", function(e) {
	Ti.API.info('open signin window*************');
	if (OS_IOS) {
...
Ti.API.info('addEventListener');

$.signin.addEventListener('android:back', function() {
	try {
		Ti.API.info('android:back SignIn');
		commonFunctions.showConfirmation(commonFunctions.L('exitApp', LangCode), [commonFunctions.L('yes', LangCode), commonFunctions.L('no', LangCode)], function(e) {
			try {
...
function signInAPISuccess(e) {
	try {
		...
		Ti.API.info('*** SIGN IN RESPONSE****  ', JSON.stringify(response));
		...
		if (response.ErrorCode == 0) {
			...
			Ti.API.info('specificationArray.Language : ', specificationArray.Language);
			...
			Ti.API.info('pointArray : ', pointArray);
			if (pointArray != null) {
				...
				// var sPoint = pointArray.SurveyPoint != null ? parseInt(pointArray.SurveyPoint) : 0;
				// var _3DPoint = pointArray._3DFigurePoint != null ? parseInt(pointArray._3DFigurePoint) : 0;
				// var catDogPoint = pointArray.CatAndDogPoint != null ? parseInt(pointArray.CatAndDogPoint) : 0;
			...
			if (specificationArray.SympSurveySlotTime != null) {
				...
				//notificationFormatTime.setHours(notificationFormatTime.getHours(), notificationFormatTime.getMinutes(), 01);
				...
				//sendLocalNotification(resultValue, notificationFormatTime, L('symptomTimeReminder'), "survey", 01);
			}
			if (specificationArray.CognTestSlotTime != null) {
				...
				//notificationFormatTime.setHours(notificationFormatTime.getHours(), notificationFormatTime.getMinutes(), 02);
				notificationFormatTime.setHours(notificationFormatTime.getHours());
				notificationFormatTime.setMinutes(notificationFormatTime.getMinutes());
				notificationFormatTime.setSeconds(2);

				Ti.API.info('notificationFormatTime1 : ', notificationFormatTime);
				notificationFormatTime = commonFunctions.getTwelveHrFormatTime(notificationFormatTime);
				Ti.API.info('notificationFormatTime : ', notificationFormatTime);
				var resltValue = {
					"userID" : specificationArray.UserID,
				...
				commonDB.insertLocalShedule(resltValue);
				// setTimeout(function() {
				// //sendLocalNotification(resultValue, notificationFormatTime, L('cognitionTimeReminder'), "cognition", 02);
				arrayItems = [];
				// }, 1000);

			}
			Ti.API.info('REPEAT ID SYMp:: ', specificationArray.SympSurveyRepeatID);
			Ti.API.info('REPEAT ID COG:: ', specificationArray.CognTestRepeatID);
			...
			var openWin = Alloy.Globals.NAVIGATION_CONTROLLER.checkWindowWExist("home");
			Ti.API.info('openWin : ', openWin);
			if (openWin == null) {
			...
			if (specificationArray.PrefferedSurveys == null || specificationArray.PrefferedSurveys == "") {
				...
				// var surveyArray = ["Mood", "Anxiety", "Thoughts", "Sleep", "Orientation", "Memory"];
				// } else {
				// var surveyArray = ["Sleep", "Orientation", "Memory"];
				// }
				...
				for (var i = 0; i < surveyIds.length; i++) {
					...
					// var surveyName = commonDB.getSurveyName(surveyIds[i]);
					// if (surveyName != null && surveyName != "") {
					// surveyTexts.push(surveyName);
					// }
			...
			if (specificationArray.PrefferedCognitions == null || specificationArray.PrefferedCognitions == "") {
				...
				//var cognitionTestArray = ["n-back Test", "trails-b Test", "Spatial Span Forward", "Spatial Span Backward", "Simple Memory Test", "Serial 7s", "3D Figure Copy", "Visual Association Task", "Digit Span Forward", "Digit Span Backward", "Cats and Dogs(New)", "Temporal Order"];
	...
	try {
		...
		if (index === 0) {
			...
			if (OS_IOS) {
				...
				// var formateTime = commonFunctions.getTwelveHrFormatTime(time);
				// Ti.API.info('formateTime : ', formateTime);
				// var timeForLocal = commonFunctions.ConvertTwentyFourHourForLocal(formateTime);
				// sendDateTime = commonFunctions.formatTimeToDate(timeForLocal, new Date(), seconds);
				// Ti.API.info('TIME FOR LOCAL:: ', timeForLocal);
				// var setTime = commonFunctions.formatTimeForAndroidNot(formateTime, new Date(), seconds);
