		...
		for (var j = 0; j < noOFBoxes; j++) {
			...
			if (highlightedItems.contains(boxNumber) == true) {
				...
				tempTargetView.addEventListener('click', function(e) {
					Ti.API.info('tempTargetView Click : ', JSON.stringify(e));
				...
				Ti.API.info('add tempTargetView : ', tempTargetView.boxNumber);
				k += 1;
				verticalView.add(tempTargetView);

			}

			// var boxLabel = Titanium.UI.createLabel({
			// color : 'black',
			// width : Ti.UI.SIZE,
			// height : Ti.UI.SIZE,
			// text : boxNumber,
			// font : Alloy.Globals.ExtraSmallFontLight
			//
			// });
			// verticalView.add(boxLabel);
