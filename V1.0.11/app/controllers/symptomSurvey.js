...
$.symptomSurvey.addEventListener("open", function(e) {
	try {
		...
		//Ti.API.info('allQuestionsArray : ', allQuestionsArray);
		// if (allQuestionsArray != null && allQuestionsArray.length != 0 && allQuestionsArray[0].surveyType == 4) {
		...
		//Ti.API.info('survey is' + args.surveyName);
...
function loadingValues(displayNumb, isBack) {
	try {
		...
		if (displayNumb == 2) {
			...
			//	Ti.API.info('allQuestionsArray[currentQuestion - 1].surveyType : ', allQuestionsArray[currentQuestion - 1].surveyType);
			...
			//Ti.API.info('Hide bottom');
			$.QuestionContentBottomView.visible = false;
		}
		//var totalQues = Math.ceil(totalQuestions / 2);

		//var totalQues = totalQuestions;
...
function setAnswerTop(answer, type, selectedRow) {
	...
	Ti.API.info('setAnswerTop : ', answer, type);
	...
	if (picker_data !== null && type == 4) {
		...
		if (Ti.Platform.osname != 'android') {
			...
			// title : picker_data[i].title,
			// value : picker_data[i].title,
			// });
			// }
			//
			// pickerControlTop.add(data);
			if (picker_data.length != 0) {
				var data = [];
				for (var i = 0; i < picker_data.length; i++) {
					data.push({
...
function pickerControlTopChangeAndroid(e) {
	...
	// for (var i = 0; i < topPickerData.length; i++) {
	// var item = e.section.getItemAt(i);
	// Ti.API.info('item : ', item, i);
	// item.properties.backgroundColor = "#fff";
	...
	setProgressBar(currentPro, totalQuestions);
	if (e.itemIndex != null) {
		Ti.API.info('topPickerData[e.itemIndex].title : ', topPickerData[e.itemIndex].title);
		Ti.API.info('e.itemIndex : ', e.itemIndex);
		//currentAnswer1 = e.rowIndex;
		//alert("answer1Distance : " + answer1Distance);
...
function pickerControlBottomChangeAndroid(e) {
	clearTimeout(scrollTimer);
	//for (var i = 0; i < bottomPickerData.length; i++) {
